<template>
  ...
  <v-sheet rounded="rounded" class="bg-transparent ma-0 pa-0 justify-center align-center h-100">
    <v-parallax :src="background" style="overflow: hidden; height: 100%; max-height: calc(100vh - 60px)">
      ...
      <v-container class="d-flex justify-center align-center fill-width rounded-lg pa-10 ma-0" fluid="true"
        v-if="width > 960" style="height: 100vh">
        <v-card class="bg-transparent w-100" :elevation="0">
          ...
          <v-row class="d-flex justify-start ga-4 pa-1">
            <v-col class="d-flex align-center justify-center pa-0 my-5 ml-2" cols="auto">
              <v-hover v-slot="{ isHovering, props }">
                <router-link to="/timetable">
                  <v-btn class="text-h5 py-1 px-8 font-weight-bold rounded-lg text-overflow ml-2 mr-6" width="100%"
                    height="80px" :color="isHovering ? 'primary-darken-1' : `success-light`" v-bind="props">
            ...
            <v-col class="d-flex align-center justify-center pa-0 my-5" cols="auto">
              <v-hover v-slot="{ isHovering, props }">
                <router-link to="/form">
                  <v-btn class="text-h5 py-1 px-8 font-weight-bold rounded-lg text-overflow mx-6" width="100%"
                    height="80px" :color="isHovering ? 'primary-darken-1' : `success-light`" v-bind="props">
        ...
        <div class="rounded-lg justify-center w-100">
          <v-col class="d-flex align-center justify-center pa-0 my-5">
            <router-link to="/timetable" class="w-100">
              <v-btn class="py-2 rounded-lg text-overflow homepage-btn" color="success-light" width="100%"
      ...
      <!-- 更多資訊 -->
      <v-fab class="mr-3 mb-6 border-0" :color="width > 960 ? `primary` : `grey-darken-4`" size="35" fixed app appear
        location="bottom end" extended prepend-icon="mdi-help">
        <v-menu activator="parent" location="top end" transition="fade-transition">
          <template v-slot:default="{ props }">
            <v-card :class="width > 960 ? 'menu-light' : 'menu-dark'" class="bg-transparent" elevation="0"
              v-bind="props">
              <v-list density="compact" max-width="200" rounded="lg" slim class="mb-4 justify-center homepage-list">
                <v-list-item class="homepage-list" title="後台登入" link />
                ...
                <span class="d-flex justify-center align-center homepage-list">
                  <v-list-item title="聯絡我們" class="text-subtitle-1 d-flex align-center justify-center mt-2">
                    <v-btn v-for="(icon, i) in icons" :key="i" :href="icon.link" size="medium" variant="plain"
                      target="_blank">
                      <v-icon :icon="icon.icon" class="align-center rounded-circle pa-4 mr-2 mt-2 homepage-list-btn" />
                    </v-btn>
                  </v-list-item>
                </span>
              </v-list>
